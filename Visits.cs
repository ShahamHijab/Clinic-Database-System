...
namespace ClinicManagementSystem
{
    public partial class Visits : Form
    {
        ...
        public Visits(int account_id, int patient_id, int reservation_id)
        {
            ...
            if (result == null) groupBox1.Enabled = true;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (command.ExecuteNonQuery() > 0) MessageBox.Show("Visit added successfully");
            else MessageBox.Show("Failed to add visit");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (command.ExecuteNonQuery() > 0) MessageBox.Show("Visit updated successfully");
            else MessageBox.Show("Failed to update the visit");
        ...
        {
            command = con.CreateCommand();
            command.CommandText = "SELECT visit_id, patient.account_id, patient.account_name, secretary.account_id, secretary.account_name, doctor.account_id, doctor.account_name, visit_date, visit_reasons, visit_diagnosis, visit_notes FROM Visit, Reservation_clinic, accounts_clinic as patient, accounts_clinic as secretary, accounts_clinic as doctor WHERE visit_reservation_id=reservation_id AND reservation_patient_id= patient.account_id AND reservation_secretary_id=secretary.account_id AND visit_doctor_id=doctor.account_id AND patient.account_id=@patient_id";
