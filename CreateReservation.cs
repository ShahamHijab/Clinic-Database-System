...
namespace ClinicManagementSystem
{
    public partial class CreateReservation : Form
    {
        ...
        {
            command = con.CreateCommand();
            command.CommandText = "SELECT reservation_visit_slot FROM Reservation_clinic WHERE reservation_visit_date=@date";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            command = con.CreateCommand();
            command.CommandText = "INSERT INTO Reservation_clinic (reservation_secretary_id, reservation_patient_id, reservation_visit_date, reservation_visit_slot, reservation_date) VALUES (@secretary_id, @patient_id, @visit_date, @visit_slot, @date)";
            ...
            {
                //Reservation performed successfully
                command.CommandText = "SELECT reservation_id FROM Reservation_clinic WHERE reservation_visit_date=@visit_date AND reservation_visit_slot=@visit_slot";
                int reservation_id = (int) command.ExecuteScalar();

                MessageBox.Show("Reservation was made successfully");
                MessageBox.Show("Reservation ID: " + reservation_id.ToString());
            }
            else
            {
                MessageBox.Show("Error while performing reservation");
            }
